...
import { AfterViewInit, Component, ElementRef, OnDestroy, OnInit, inject, input, viewChild } from '@angular/core';
...
import { Subject, debounceTime, distinctUntilChanged, filter, first, switchMap, takeUntil } from 'rxjs';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatCheckboxModule, MatIconModule, MatButtonModule, MatInputModule, MatFormFieldModule, ReactiveFormsModule, RouterModule],
...
export class InboxItemComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.itemBody.valueChanges.pipe(
      filter(Boolean),
