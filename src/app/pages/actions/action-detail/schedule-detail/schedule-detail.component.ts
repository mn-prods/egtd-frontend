...
import { ActionDocument, ActionType, Schedule, Waiting } from 'src/app/db/entities/action.entity';
...
export class ScheduleDetailComponent implements ActionItem, OnInit, AfterViewInit {
  ...
  async ngOnInit() {
    this.selectedDate = this.action().schedule?.on || null;

    this.schedule = new FormGroup<FormGroupValue<Schedule>>({
    ...
    this.schedule.valueChanges
      ...
      .subscribe((updatedAction) => {
        ...
        if (!updatedAction.schedule?.on) return;

        this.selectedDate = updatedAction.schedule.on;
  ...
  setScheduleOn(selectedDate: Date | null) {
    if (!selectedDate) return;

    this.schedule.patchValue({ on: selectedDate });
  }
